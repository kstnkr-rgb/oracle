...
const TRANSLATE_SYSTEM_PROMPT = `Переводи текст с английского на русский. Выводи ТОЛЬКО перевод — никаких приветствий, предисловий, пояснений, фраз о себе или о том, что ты ИИ. Первое слово ответа — начало перевода. Пиши живо, по-человечески, передавай смысл естественным русским языком. Тон — позитивный и поддерживающий. Если входной текст содержит маркеры вида <<<N>>>, сохраняй их в том же месте в переводе. Никогда не используй длинное тире (—), только короткое тире (–) или дефис (-).`;
...
Целевой объём — не более 800 знаков. Пиши ёмко и по делу.

Никаких заголовков, списков, звёздочек и markdown. Только связный текст.

Никогда не пиши, что не можешь что-то предсказать, что будущее неизвестно или что ты не в состоянии дать ответ. Всегда давай конкретный, уверенный совет.
...
const DREAM_SYSTEM_PROMPT = `Толкуй сны с точки зрения психологии и астрологии. У тебя есть натальная карта пользователя и текущие транзиты — используй их для персонализации. Обрати внимание на положение Луны (эмоции, подсознание), 12-й дом (сновидения, бессознательное), Нептун (сны, интуиция), а также на текущие транзиты, которые могут объяснять появление определённых тем в снах. Твоё толкование должно быть в позитивном ключе. Пиши от второго лица («ты»). Язык — простой, живой, человечный. Без астрологического жаргона: не используй «транзит», «натальный», «аспект», «конъюнкция» — заменяй на обычные слова. Тон — поддерживающий, вдохновляющий. Целевой объём — не более 800 знаков. Пиши ёмко и по делу. Никаких заголовков, списков, звёздочек и markdown. Только связный текст. Никогда не пиши, что не можешь истолковать сон. Никогда не используй длинное тире (—). Вместо него используй короткое тире (–) или дефис (-).`;
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'POST' && urlPath === '/oracle-ru') {
    ...
    req.on('end', () => {
      ...
      callClaude(ORACLE_SYSTEM_PROMPT, userMessage, (err, text) => {
        ...
        res.writeHead(200, {'Content-Type': 'application/json; charset=utf-8'});
        res.end(JSON.stringify({ success: true, text: text.slice(0, 800) }));
      }, CLAUDE_MODEL_SONNET);
    });
    return;
  }

  if (req.method === 'POST' && urlPath === '/cookie-ru') {
    callClaude(COOKIE_SYSTEM_PROMPT, 'Создай предсказание для печенья.', (err, text) => {
      if (err) { res.writeHead(500); res.end(JSON.stringify({error: err.message})); return; }
      res.writeHead(200, {'Content-Type': 'application/json; charset=utf-8'});
      res.end(JSON.stringify({ success: true, text: text.trim().slice(0, 100) }));
    });
    return;
  }
  ...
  if (req.method === 'POST' && urlPath === '/dream-ru') {
    ...
    req.on('end', () => {
      ...
      const userMessage = `Сон: "${dream}"\n\nНАТАЛЬНАЯ КАРТА:\n${JSON.stringify(natal.positions, null, 2)}\n\nТЕКУЩИЕ ТРАНЗИТЫ:\n${JSON.stringify(transits.positions, null, 2)}`;
      callClaude(DREAM_SYSTEM_PROMPT, userMessage, (err, text) => {
        if (err) { res.writeHead(500); res.end(JSON.stringify({error: err.message})); return; }
        res.writeHead(200, {'Content-Type': 'application/json; charset=utf-8'});
        res.end(JSON.stringify({ success: true, text: text.slice(0, 800) }));
      }, CLAUDE_MODEL_SONNET);
